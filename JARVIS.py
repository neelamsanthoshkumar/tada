...
class ModernJARVIS:
    def __init__(self):
        ...
        self.groq_api_keys = [
            ...
            os.getenv('GROQ_API_KEY_SECONDARY')   # Secondary key
        ]
        self.current_groq_key_index = 0
        self.groq_model = "compound-beta"
        ...
        self.image_models_url = "https://api.a4f.co/v1/models"
        self.image_generate_url = "https://api.a4f.co/v1/images/generations"
        ...
        self.image_models = [
            ...
            "provider-2/dream-sharper",
            "provider-2/proteus",
            "provider-2/flux.1-schnell"
        ]

        # Hugging Face Image Generation API Configuration (Alternative)
        self.huggingface_api_keys = [os.getenv('HUGGINGFACE_API_KEY')]
        self.current_huggingface_key_index = 0
        # Updated Hugging Face models, prioritizing newer/better free models
        self.huggingface_models = [
           "stabilityai/stable-diffusion-3.5-large",
           "stabilityai/stable-diffusion-3-medium-diffusers", # SD3 Medium
           "ByteDance/SDXL-Lightning", # Very fast XL model
           "segmind/SSD-1B", # Good 1B param XL model
           "stabilityai/stable-diffusion-xl-base-1.0", # Standard SDXL
           "runwayml/stable-diffusion-v1-5", # Reliable older model
...
function addMessageActions(messageDiv) {
    ...
    if (messageDiv.classList.contains('message-bot')) {
        if (!messageDiv.querySelector('.generated-image') && !messageDiv.querySelector('.tts-controls')) {
            ...
            tempDiv.querySelectorAll('.message-model, .message-time, .message-actions, .search-results, .open-all-btn, .code-actions-container, .code-output-area, .attached-file-indicator, .katex-display, .katex, .message-actions-feedback, .tts-controls, .download-btn').forEach(el => el.remove());
